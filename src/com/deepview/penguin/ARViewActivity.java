...
public abstract class ARViewActivity extends Activity implements
	...
	public void onSurfaceCreated() {
		try {
			...
			Log.i("onSurfaceCreated", "++++++regist OK");
		} catch (Exception e) {
			Log.e("onSurfaceCreated", "++++++"+e.toString());
		}
	}
	...
	private void createMobileSDK() {
		try {
			...
			// Make sure to provide a val5id application signature
		...
		try {
			...
			if (poses.size() > 0) {
				...
				if (cosID != mDetectedCosID) {
					UnifeyeDebug.log("detected ^H^ ^H^ ^H^ ^H^ " + cosID);
